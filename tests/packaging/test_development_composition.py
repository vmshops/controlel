...
def test_development_bundle_contains_matching_frontend_and_core_without_mutating_release_manifest(
    ...
    output = tmp_path / "controlel-dev-0.15.0.zip"
    ...
    assert composition["integration"]["release_source_requirement"] == "controlel==0.14.0"
    assert composition["integration"]["development_requirement"] == "controlel==0.15.0"
...
def test_development_bundle_rejects_a_relabelled_or_mismatched_core(tmp_path: Path) -> None:
    ...
    with pytest.raises(DevelopmentCompositionError, match="expected controlel 0.15.0"):
        build_development_composition(
            tmp_path / "wrong.zip",
            source_ref="test-source-ref",
