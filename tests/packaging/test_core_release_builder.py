...
def _write_varying_sdist(
    ...
    *,
    ...
    members = [
        ("controlel-0.2.0", None),
        ("controlel-0.2.0/PKG-INFO", b"Metadata-Version: 2.4\nName: controlel\nVersion: 0.2.0\n"),
        (
            "controlel-0.2.0/src/controlel/application/services/temperature_hysteresis_policy.py",
            b'"""Policy."""\n',
        ),
    ]
    if reverse_members:
...
def test_canonicalize_sdist_removes_all_container_metadata_variance(
    ...
    with tarfile.open(first, mode="r:gz") as archive:
        ...
        pkg_info = archive.extractfile("controlel-0.2.0/PKG-INFO")
        assert pkg_info is not None
        assert b"Version: 0.2.0" in pkg_info.read()
