...
def test_manifest_pins_published_core_and_keeps_version_independent() -> None:
    ...
    assert manifest["requirements"] == ["controlel==0.5.0"]
...
def test_framework_ci_separates_local_source_and_public_core_compositions() -> None:
    ...
    assert "controlel==0.5.0" in workflow
...
def test_public_core_provenance_records_history_and_current_composition_hash() -> None:
    ...
    assert "controlel-0.5.0-py3-none-any.whl" in checker
    assert "bb6e1e4a7b802e1ebc3d65bbd18c2111f6a4b992c3aaac56a3a823f272f45d09" in checker
