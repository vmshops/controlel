...
def test_hacs_source_contract_is_exact() -> None:
    ...
    assert not any(name.startswith("frontend/tests/") for name in files)
    ...
    assert "client.canonicalizeDraft({" in wizard_source
    assert "client.activateRevision({" in wizard_source
    ...
    for obsolete_route in ("start", "reopen", "update", "validate", "delete"):
        assert "createSetupWizardClient" in api_client_source
        assert f'"controlel/setup/write/v1/{obsolete_route}"' not in wizard_source
