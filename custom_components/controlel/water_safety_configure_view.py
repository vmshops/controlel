...
WaterSafetyLifecycle = Literal["not_configured", "draft_incomplete", "configured"]
...
def water_safety_menu_summary(view: WaterSafetyConfigureView) -> str:
    ...
    if view.lifecycle == "draft_incomplete":
        ...
        return f"Water Safety has an incomplete draft ({draft_id}). Native section editing is not available yet."
...
def _draft_detail(view: WaterSafetyConfigureView, section: WaterSafetySection) -> str:
    ...
    if section == "status":
        ...
        return (
            f"Draft incomplete ({draft.draft_id}, revision {draft.revision}). "
...
def _payload_detail(
    ...
    *,
    ...
    moisture_binding = _binding_locator(draft, WATER_SAFETY_SENSOR_ROLE) if draft is not None else None
    ...
    if section == "area_sensor":
        sensor = moisture_binding or "Not selected"
        return f"Area: {payload.area_name} ({payload.area_id}). Moisture sensor: {sensor}."
