"""Native Home Assistant authoring for Water Safety siren outputs."""
...
class WaterSafetySirensEditor:
    """One discovery/editor snapshot for the native siren form."""
...
async def async_load_water_safety_sirens_editor(
    hass: Any,
    ...
    *,
    ...
    """Load persisted Water intent plus conservative HA siren candidates."""
    ...
    selected = tuple(
        ...
        if binding.role.startswith(SIREN_ROLE_PREFIX)
    ...
    candidates = _siren_candidates(discovery, (DEFAULT_SIREN_ROLE,))
    compatible = tuple(
        sorted(
            locator
            for candidate in candidates
            if _SIREN_DOMAIN_REASON in candidate.reason_codes
    ...
    return WaterSafetySirensEditor(
        discovery=discovery,
...
async def async_save_water_safety_sirens_draft(
    repository: HomeAssistantSetupRepository,
    ...
    *,
    ...
    if incompatible:
        raise ValueError(f"selected siren target is incompatible or unavailable: {incompatible[0]}")
    ...
    retained_bindings = [item for item in bindings if not item.role.startswith(SIREN_ROLE_PREFIX)]
    assignments = _role_assignments(selected_targets, existing_roles)
    roles = tuple(role for role, _target in assignments)
    candidates = _siren_candidates(editor.discovery, roles)
    candidate_by_role_target = {
        (candidate.role, candidate.reference.current_locator): candidate
        for candidate in candidates
        if _SIREN_DOMAIN_REASON in candidate.reason_codes and candidate.reference.current_locator is not None
    }
    siren_bindings: list[BindingSelection] = []
    for role, target in assignments:
        candidate = candidate_by_role_target.get((role, target))
        if candidate is None:
            raise ValueError(f"selected siren target is not compatible: {target}")
        siren_bindings.append(
            BindingSelection(
                role=role,
...
def _role_assignments(
    ...
    for target in target_ids:
        ...
        if role is None or role in assigned_roles:
            if DEFAULT_SIREN_ROLE not in assigned_roles and DEFAULT_SIREN_ROLE not in existing_roles.values():
                role = DEFAULT_SIREN_ROLE
            else:
                digest = hashlib.sha256(target.encode("utf-8")).hexdigest()[:12]
                role = f"{SIREN_ROLE_PREFIX}target_{digest}"
        if role in assigned_roles:
            raise ValueError("siren target role collision")
...
def _canonical_targets(target_ids: Sequence[str]) -> tuple[str, ...]:
    ...
    if len(targets) > MAX_SIREN_TARGETS:
        raise ValueError("too many siren targets")
    if len(set(targets)) != len(targets):
        raise ValueError("siren targets must be unique")
    if any(not target.startswith("siren.") or target.count(".") != 1 for target in targets):
        raise ValueError("siren targets must be Home Assistant siren entities")
    return targets
