...
class OperationalEventRecorder:
    ...
    def command_dispatched(self, action: HeatingAction, timestamp: datetime, *, correlation_id: str) -> None:
    ...
    def command_failed(
        ...
        *,
        ...
        correlation = correlation_id or self.command_requested(action, timestamp)
        self._emit(
            timestamp,
            Category.SOURCE_CONTROL,
            Severity.WARNING,
    ...
    def supervision(self, state: RuntimeSupervisionState, timestamp: datetime) -> None:
        ...
        if previous.phase is not state.phase:
            if state.phase is SupervisorPhase.FAILSAFE:
                ...
                self._emit(
                    timestamp, Category.SUPERVISION, Severity.NOTICE, Code.FAILSAFE_EXITED, correlation_id=correlation
                )
                self._emit(
                    timestamp, Category.RUNTIME, Severity.NOTICE, Code.RUNTIME_RECOVERED, correlation_id=correlation
                )
                self._runtime_fatal_active = False
    ...
    def _record_safety(self, result: HeatDemandEvaluationResult, timestamp: datetime) -> None:
        ...
        if phase is HeatDemandSafetyPhase.INDETERMINATE_GRACE:
            self._emit(timestamp, Category.SAFETY, Severity.WARNING, Code.SAFETY_GRACE_STARTED)
        elif phase is HeatDemandSafetyPhase.INDETERMINATE_TIMED_OUT:
            self._emit(timestamp, Category.SAFETY, Severity.WARNING, Code.SAFETY_GRACE_EXPIRED)
    ...
    def _record_reconciliation(self, result: HeatDemandEvaluationResult, timestamp: datetime) -> None:
        ...
        if status in drift_states and previous not in drift_states:
            self._emit(timestamp, Category.SOURCE_RESILIENCE, Severity.WARNING, Code.SOURCE_DRIFT_DETECTED)
        if status is SourceReconciliationStatus.CORRECTION_REQUIRED and previous is not status:
            self._emit(timestamp, Category.SOURCE_RESILIENCE, Severity.NOTICE, Code.SOURCE_RECONCILIATION_STARTED)
        if previous in drift_states and status not in drift_states:
            self._emit(timestamp, Category.SOURCE_RESILIENCE, Severity.NOTICE, Code.SOURCE_RECONCILIATION_COMPLETED)
        if result.status is HeatDemandEvaluationStatus.RESILIENCE_COMMAND_HELD and previous is not status:
            self._emit(timestamp, Category.SOURCE_RESILIENCE, Severity.WARNING, Code.CORRECTIVE_ACTION_HELD)
    ...
    def _record_command(self, result: HeatDemandEvaluationResult, timestamp: datetime) -> None:
        ...
        self.command_requested(action, timestamp, correlation_id=correlation)
        self.command_dispatched(action, timestamp, correlation_id=correlation)
