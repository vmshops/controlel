...
current candidate is `0.2.0`; it is not a second source for the core package
version and evolves independently. Integration `0.1.1` remains immutable and
published.
...
Integration `0.2.0` pins exactly `controlel==0.1.0`. CI keeps local editable
...
- manifest and `INTEGRATION_VERSION`: `0.2.0`;
- future integration tag: `v0.2.0`;
- GitHub Release name:
  `Controlel Home Assistant Integration v0.2.0`;
- HACS asset: `controlel.zip`;
- checksum asset: `controlel.zip.sha256`;
- exact core dependency: `controlel==0.1.0`.

Integration `0.1.1` is already published. No `v0.2.0` tag or GitHub Release
...
6. Create annotated tag `v0.2.0` at the reviewed commit.
...
10. Verify public core dependency installation, config flow, setup, unload,
    ...
    removal. Confirm an existing `0.1.1` entry loads without migration.
