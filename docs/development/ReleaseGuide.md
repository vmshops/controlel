...
## Prepared Core 0.18.0 candidate

Core `0.18.0` is prepared from current mainline as the next Core release
candidate after immutable public Core `0.17.0`. It packages module-scoped
active-reference persistence/resolution, Water shutoff contracts, optional
empty notification/output role lists that remain activation-ready, and Water
Safety evidence/snapshot persistence isolation. UNKNOWN and UNAVAILABLE remain
never dry evidence, and accepted output requests never claim physical output
state.

The candidate preserves the current Heating runtime and canonical configuration
v3 behavior. It does not change the Home Assistant `0.14.0` candidate or that
candidate's exact dependency on published `controlel==0.17.0`. Core `0.18.0`
remains untagged and unpublished until the later publication sequence is
separately approved.
...
Core `0.16.0` is published and immutable from `core-v0.16.0`. It provides the
canonical configuration v3 backend for Configure, Setup Wizard, and Heating
settings. Its public wheel and sdist identities remain recorded unchanged in
`release-metadata/releases.yaml` and `docs/releases/core-0.16.0.md`.
...
integration `0.12.0` is published and immutable from `v0.12.0`.
...
entries and the legacy runtime path remain compatible; the release does not
silently convert or merge them with canonical Setup. No config-entry migration
is required. The read-only panel and Frontend API v1 composition belong to the
...
The HA boundary adds only validated modular configuration, an event-loop notify
transport, one coalesced drain task, unload invalidation, and bounded redacted
diagnostics. Event mapping, deduplication, cursor/overflow accounting, and rate
limits remain core-owned. Notifications are disabled with no recipients by
default. There is no polling, retry loop, persistence, automatic target
discovery, or control-path influence. The custom sidebar UI and recipient
write/test APIs remain deferred.

The public wheel identity is `controlel-0.8.0-py3-none-any.whl`, 141,379 bytes,
SHA-256 `b9f12d0fadf8a0a53e7bd102fc707bf2b518e776b92a2cdbee240562b4079d8f`.
The public sdist is `controlel-0.8.0.tar.gz`, 89,285 bytes, SHA-256
`3aa051d187ab5b5305584f67893ed8a89c82bd7061fa3a28ecc4f9136b0fa84c`.

## Milestone 31A core 0.7.0 boundary (historical)
...
## Integration 0.9.0 M31A diagnostics boundary (historical)
...
## Published core 0.8.0 record

- Exact release commit: `a8d44bb210db4727fc381acc439334cb2b49d8bb`.
- Annotated tag: `core-v0.8.0`.
- Wheel: `controlel-0.8.0-py3-none-any.whl`, 141,379 bytes, SHA-256
  `b9f12d0fadf8a0a53e7bd102fc707bf2b518e776b92a2cdbee240562b4079d8f`.
- Sdist: `controlel-0.8.0.tar.gz`, 89,285 bytes, SHA-256
  `3aa051d187ab5b5305584f67893ed8a89c82bd7061fa3a28ecc4f9136b0fa84c`.

The package was independently installed from public PyPI without cache. M31B,
M31A, runtime-supervision, and failsafe imports resolved from isolated
`site-packages` without editable or direct-URL metadata.
...
6. Create annotated tag `v0.13.0` at the reviewed commit.
