...
M31B now adds an unreleased core smart-notification foundation as a passive
...
port. It adds no polling, retries, persistence, host adapter/configuration, or
control influence. A separate release-boundary change will prepare core 0.8.0;
host integration remains a later independent change. M31C may add explainable
statistics/analytics and must preserve the command/report/physical-reality
distinctions.
