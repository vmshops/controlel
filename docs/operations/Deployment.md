...
The integration source is `custom_components/controlel`. The `0.14.0` candidate
declares one config entry and requires published `controlel==0.16.0`.
...
Public-package validation instead installs `controlel==0.16.0` into a separate
...
manually. The implementation is the unpublished `0.14.0`
integration candidate; deterministic release packaging remains a
...
Core `0.16.0` is public and immutable. The integration `0.14.0` candidate
installs exactly `controlel==0.16.0`; public-package CI verifies the Setup,
