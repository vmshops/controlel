...
Integration 0.8.2 supplies that composition with the existing serialized HA
...
`0.8.2` declares one config entry and requires published
`controlel==0.6.0`.
...
unpublished `0.8.2` candidate. No default
...
Core `0.6.0` is public and immutable. Integration `0.8.2` installs exactly
