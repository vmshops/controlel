...
published release; `0.11.0` remains an unpublished candidate.
...
create or update one stable ERROR Repairs issue. In 0.9.0 the supervisor
...
Unload first rejects new state and timer submissions, removes listeners, cancels
all adapter-owned one-shot timers, lets
...
Integration candidate `0.11.0` requires exactly `controlel==0.10.0`. Normal supported
...
environment, install `controlel==0.10.0` from PyPI, and resolve it from
`site-packages`.
...
`controlel==0.10.0` from PyPI for the isolated public composition.
